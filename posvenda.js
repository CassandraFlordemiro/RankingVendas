...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('filtroBusca').addEventListener('input', filtrarTratativas);
  document.getElementById('filtroConsultorPos').addEventListener('change', filtrarTratativas);
  document.getElementById('filtroStatus').addEventListener('change', filtrarTratativas);
  document.getElementById('btnExportarPlanilha').addEventListener('click', exportarPlanilhaPos);
...
function filtrarTratativas() {
  ...
  vendasFiltradasAtuais = vendasPosCache.filter(v => {
    ...
    const matchTexto = cliente.includes(termo) || matricula.includes(termo) || consultor.toLowerCase().includes(termo);
    const matchConsultor = filtroConsultor === 'TODOS' || consultor === filtroConsultor;
...
function renderizarTabelaPos() {
  ...
  tbody.innerHTML = listaFatiada.map(v => {
    ...
    const etapas = v.etapasPosVenda || { ligacao: false, linkEnviado: false, docsRecebidos: false };
...
window.alterarEtapa = async function(id, etapa, valor) {
  ...
  const etapas = dados.etapasPosVenda || { ligacao: false, linkEnviado: false, docsRecebidos: false };
  etapas[etapa] = valor;

  const todasConcluidas = etapas.ligacao && etapas.linkEnviado && etapas.docsRecebidos;
...
function exportarPlanilhaPos() {
  ...
  const linhas = vendasPosCache.map(v => {
    ...
    return {
      ...
      'FORMA DE PAGAMENTO': v.formaPagamento || v['FORMA DE PAGAMENTO'] || 'NÃO INFORMADO',
  ...
  XLSX.writeFile(wb, `PosVenda_Export_${new Date().toISOString().slice(0, 10)}.xlsx`);
}