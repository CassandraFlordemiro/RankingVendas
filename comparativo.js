...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('filtroMesComparativo').addEventListener('change', processarComparativo);
  document.getElementById('btnCompararTop3').addEventListener('click', selecionarTop3);
  document.getElementById('btnCompararTodos').addEventListener('click', selecionarTodos);
...
function renderizarCardsSelecao() {
  ...
  container.innerHTML = consultoresDisponiveis.map(c => {
...
function selecionarTop3() {
  ...
  vendasCache.forEach(v => {
    ...
    const ehRecusado = st.includes('NÃO') || st.includes('NAO') || st.includes('RECUS') || st.includes('CANCEL');
  ...
  const top3 = Object.entries(contagem).sort((a, b) => b[1] - a[1]).slice(0, 3).map(i => i[0]);
...
function processarComparativo() {
  ...
  vendasCache.forEach(v => {
    ...
    if (ehRecusado) {
      ...
      if (pag.includes('CRÉD') || pag.includes('CRED')) mapa[c].pagamentos.credito++;
      else if (pag.includes('DÉB') || pag.includes('DEB')) mapa[c].pagamentos.debito++;
  ...
  const arrayComparativo = Object.values(mapa).sort((a, b) => b.efetivadas - a.efetivadas || b.lancadas - a.lancadas);

  atualizarTopCards(arrayComparativo);
  atualizarMetasComparativo(arrayComparativo, mesFiltro);
  renderizarGraficosExecutivos(arrayComparativo);
  renderizarHeatmap(arrayComparativo);
...
function atualizarTopCards(dados) {
  if (dados.length === 0) {
    ...
    document.getElementById('cardTotalGrupoLancadas').textContent = 'de 0 lançadas';
    document.getElementById('cardReceitaGrupo').textContent = 'R$ 0,00';
    return;
  }

  const liderVendas = dados[0];
  document.getElementById('cardLiderVendas').textContent = formatarNomeCurto(liderVendas.consultor);
  document.getElementById('cardLiderVendasQtd').textContent = `${liderVendas.efetivadas} vendas efetivadas`;

  const ordenadosPorConversao = [...dados].filter(d => d.lancadas > 0).sort((a, b) => (b.efetivadas / b.lancadas) - (a.efetivadas / a.lancadas));
  if (ordenadosPorConversao.length > 0) {
    const liderConv = ordenadosPorConversao[0];
    const taxaConv = Math.round((liderConv.efetivadas / liderConv.lancadas) * 100);
    document.getElementById('cardLiderConversao').textContent = formatarNomeCurto(liderConv.consultor);
    document.getElementById('cardLiderConversaoTaxa').textContent = `${taxaConv}% de efetivação`;
  }
  ...
  document.getElementById('cardTotalGrupoEfetivadas').textContent = grupoEfetivadas;
  document.getElementById('cardTotalGrupoLancadas').textContent = `de ${grupoLancadas} lançadas`;
  document.getElementById('cardReceitaGrupo').textContent = `R$ ${grupoReceita.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
}
...
function atualizarMetasComparativo(dados, mesFiltro) {
  ...
  dados.forEach(d => {
    ...
    const metaObj = metasCache[chave] || { metaFaturamento: 7000, metaQtd: 100 };
  ...
  document.getElementById('barraProgressoMetaComparativo').style.width = `${pctFat}%`;
  document.getElementById('txtValorAtualMetaComparativo').textContent = `R$ ${atualFat.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  document.getElementById('txtValorObjetivoMetaComparativo').textContent = `R$ ${totalMetaFat.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  document.getElementById('txtPercentualMetaComparativo').textContent = `${pctFat}% Concluído`;

  const restFat = totalMetaFat - atualFat;
  document.getElementById('txtSubMetaComparativoFat').textContent = restFat > 0 ? `Faltam R$ ${restFat.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} para a meta combinada` : '🚀 META DO GRUPO BATIDA!';

  const pctQtd = Math.min(100, Math.round((atualQtd / totalMetaQtd) * 100));
  document.getElementById('barraProgressoMetaComparativoQtd').style.width = `${pctQtd}%`;
  document.getElementById('txtQtdAtualMetaComparativo').textContent = `${atualQtd} vendas`;
  document.getElementById('txtQtdObjetivoMetaComparativo').textContent = `${totalMetaQtd} vendas`;
  document.getElementById('txtPercentualMetaComparativoQtd').textContent = `${pctQtd}% Concluído`;

  const restQtd = totalMetaQtd - atualQtd;
  document.getElementById('txtSubMetaComparativoQtd').textContent = restQtd > 0 ? `Faltam ${restQtd} contratos combinados` : '🎯 META DE CONTRATOS BATIDA!';
}

// Gráficos Horizontais com Barras Arredondadas (Estilo Dashboard Executivo)
function renderizarGraficosExecutivos(dados) {
  const nomesLabels = dados.map(d => formatarNomeCurto(d.consultor));

  // 1. Barras Horizontais de Volume
  const ctxBarras = document.getElementById('graficoBarrasComparativo').getContext('2d');
  ...
  chartBarrasInstancia = new Chart(ctxBarras, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'bottom', labels: { color: '#f8f8f2', font: { size: 10 } } }
      },
      scales: {
        x: { ticks: { color: '#6272a4', precision: 0 }, grid: { color: 'rgba(255,255,255,0.04)' } },
        y: { ticks: { color: '#f8f8f2', font: { size: 11, weight: '600' } }, grid: { display: false } }
      }
    }
  });

  // 2. Barras Horizontais de Conversão
  const ctxDireita = document.getElementById('graficoRadarComparativo').getContext('2d');
  if (chartDireitaInstancia) chartDireitaInstancia.destroy();

  const taxas = dados.map(d => d.lancadas > 0 ? Math.round((d.efetivadas / d.lancadas) * 100) : 0);
...
// Renderiza a Matriz Heatmap
function renderizarHeatmap(dados) {
  const tbody = document.getElementById('corpoHeatmapComparativo');
  if (!tbody) return;

  if (dados.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--text-muted); padding: 24px;">Nenhum dado para o período.</td></tr>`;
    return;
  }

  function getHeatClass(val) {
    if (val >= 20) return 'heat-cell heat-high';
    if (val >= 5) return 'heat-cell heat-med';
    if (val > 0) return 'heat-cell heat-low';
    return 'heat-cell heat-zero';
  }

  tbody.innerHTML = dados.map(item => {
    const nomeCurto = formatarNomeCurto(item.consultor);
    const p = item.pagamentos;

    return `
      <tr>
        <td><strong>${nomeCurto}</strong></td>
        <td class="text-center ${getHeatClass(p.credito)}">${p.credito}</td>
        <td class="text-center ${getHeatClass(p.debito)}">${p.debito}</td>
        <td class="text-center ${getHeatClass(p.pix)}">${p.pix}</td>
        <td class="text-center ${getHeatClass(p.outros)}">${p.outros}</td>
        <td class="text-center font-bold text-green">${item.efetivadas} vendas</td>
      </tr>
    `;
  }).join('');
}
...
function montarTabela(dados) {
  ...
  tbody.innerHTML = dados.map((item, idx) => {
    const taxa = item.lancadas > 0 ? Math.round((item.efetivadas / item.lancadas) * 100) : 0;
    const nomeCurto = formatarNomeCurto(item.consultor);
    const posicao = idx + 1;
