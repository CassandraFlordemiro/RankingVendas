...
let metaMensalDinheiro =
  parseFloat(localStorage.getItem("ranking_meta_mensal")) || 70000.0;
let metaMensalQtd =
  parseInt(localStorage.getItem("ranking_meta_qtd"), 10) || 1000;
...
function inicializarFiltrosRetroativos() {
  ...
  document.getElementById("txtMesMetaDinheiro").textContent =
    meses[agora.getMonth()];
  document.getElementById("txtMesMetaQtd").textContent =
    meses[agora.getMonth()];
}
...
function configurarEdicaoMetas() {
  ...
  if (btnEditarDinheiro) {
    btnEditarDinheiro.onclick = () => {
      const valorDigitado = prompt(
        "Informe a nova meta financeira em R$:",
        metaMensalDinheiro,
      );
  ...
  if (btnEditarQtd) {
    btnEditarQtd.onclick = () => {
      const valorDigitado = prompt(
        "Informe a nova meta de contratos (número de vendas):",
        metaMensalQtd,
      );
      if (valorDigitado !== null) {
  ...
  db.collection("vendas").onSnapshot(
    (snapshot) => {
      ...
      snapshot.forEach((doc) =>
        vendasCacheLocal.push({ id: doc.id, ...doc.data() }),
      );
...
function processarRankings(vendas) {
  ...
  const mesFiltroIndex = parseInt(
    document.getElementById("filtroMesRanking").value,
    10,
  );
  ...
  vendas.forEach((v) => {
    ...
    const ehEfetivada =
      (status.includes("CONCLU") || status === "OK") && !ehRecusado;

    // Diário
    if (matchDia) {
      if (!mapaDiario[consultor]) {
        mapaDiario[consultor] = {
          consultor,
          lancadas: 0,
          efetivadas: 0,
          valorTotal: 0,
  ...
  const arrayDiario = Object.values(mapaDiario).sort(
    (a, b) => b.efetivadas - a.efetivadas || b.lancadas - a.lancadas,
  );
  const arrayMensal = Object.values(mapaMensal).sort(
    (a, b) => b.efetivadas - a.efetivadas || b.lancadas - a.lancadas,
  );

  // Atualiza Hero
  document.getElementById("heroTotalHoje").textContent =
    `${totalHojeVendas} vendas`;
  document.getElementById("heroValorHoje").textContent =
    `R$ ${totalHojeValor.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`;
  document.getElementById("heroLiderHoje").textContent =
    arrayDiario.length > 0 && arrayDiario[0].efetivadas > 0
      ? formatarNomeCurto(arrayDiario[0].consultor)
      : "Em disputa...";

  document.getElementById("badgeTotalDiario").textContent =
    `${totalHojeVendas} Confirmadas`;
  document.getElementById("badgeTotalMensal").textContent =
    `${totalMensalVendas} Confirmadas`;
...
function atualizarTermometrosDuplos(valorAcumulado, qtdAcumulada) {
  ...
  const pctDinheiro = Math.min(
    100,
    Math.round((valorAcumulado / metaMensalDinheiro) * 100),
  );
  const barraDinheiro = document.getElementById("barraProgressoMeta");

  barraDinheiro.style.width = `${pctDinheiro}%`;
  document.getElementById("txtValorAtualMeta").textContent =
    `R$ ${valorAcumulado.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`;
  document.getElementById("txtValorObjetivoMeta").textContent =
    `R$ ${metaMensalDinheiro.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`;
  document.getElementById("txtPercentualMeta").textContent =
    `${pctDinheiro}% Concluído`;

  const restanteDinheiro = metaMensalDinheiro - valorAcumulado;
  if (restanteDinheiro > 0) {
    document.getElementById("txtSubMetaDinheiro").textContent =
      `Faltam R$ ${restanteDinheiro.toLocaleString("pt-BR", { minimumFractionDigits: 2 })} para a meta`;
  } else {
    document.getElementById("txtSubMetaDinheiro").textContent =
      `🚀 META BATIDA!`;
  }
  ...
  const pctQtd = Math.min(
    100,
    Math.round((qtdAcumulada / metaMensalQtd) * 100),
  );
  const barraQtd = document.getElementById("barraProgressoMetaQtd");

  barraQtd.style.width = `${pctQtd}%`;
  document.getElementById("txtQtdAtualMeta").textContent =
    `${qtdAcumulada} vendas`;
  document.getElementById("txtQtdObjetivoMeta").textContent =
    `${metaMensalQtd} vendas`;
  document.getElementById("txtPercentualMetaQtd").textContent =
    `${pctQtd}% Concluído`;

  const restanteQtd = metaMensalQtd - qtdAcumulada;
  if (restanteQtd > 0) {
    document.getElementById("txtSubMetaQtd").textContent =
      `Faltam ${restanteQtd} contratos para a meta`;
  } else {
    document.getElementById("txtSubMetaQtd").textContent =
...
function atualizarPodioVisual(tipo, lista) {
  ...
  if (c1 && c1.efetivadas > 0) {
    ...
    document.getElementById(`score${tipo}1`).textContent =
      `${c1.efetivadas} vendas (R$ ${c1.valorTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2 })})`;
    document.getElementById(`avatar${tipo}1`).textContent = nome1
      .split(" ")
      .map((p) => p[0])
      .slice(0, 2)
      .join("");
  ...
  if (c2 && c2.efetivadas > 0) {
    ...
    document.getElementById(`score${tipo}2`).textContent =
      `${c2.efetivadas} vendas`;
    document.getElementById(`avatar${tipo}2`).textContent = nome2
      .split(" ")
      .map((p) => p[0])
      .slice(0, 2)
      .join("");
  ...
  if (c3 && c3.efetivadas > 0) {
    ...
    document.getElementById(`score${tipo}3`).textContent =
      `${c3.efetivadas} vendas`;
    document.getElementById(`avatar${tipo}3`).textContent = nome3
      .split(" ")
      .map((p) => p[0])
      .slice(0, 2)
      .join("");
